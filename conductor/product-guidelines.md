...
- **Deployment Payload Size limits:** Agent Engine has a strict 8 MB payload limit for the source code zip. Because `adk deploy agent_engine` uses `shutil.copytree` to package the working directory, **every agent project directory must include an `.ae_ignore` file** explicitly ignoring `.venv`, `__pycache__`, `.env`, `.adk`, and `uv.lock`. Failure to include this will result in massive payload rejections from Vertex AI. *(Note: Standard `.gitignore` or `.gcloudignore` files are NOT respected by the Agent Engine deployer).*
...
- **Requirements Generation:** Because `adk deploy agent_engine` packages and installs directly from `requirements.txt` in a clean Linux environment, the `requirements.txt` file must contain the **full dependency tree**, including `google-adk`. If using `uv`, you must generate it prior to deployment via `uv export --no-hashes --format requirements-txt > requirements.txt`. Hand-written, partial requirement files will cause Vertex AI build failures (Error Code 3).
